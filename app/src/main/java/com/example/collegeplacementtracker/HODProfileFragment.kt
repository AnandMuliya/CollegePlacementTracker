...
class HODProfileFragment : Fragment() {
                ...
                if (email != null) {
                    ...
                    view.post {
                        currentUser?.let { user ->
                            ...
                            view.findViewById<TextView>(R.id.profileNameTextView)?.text = user.fullName
                            view.findViewById<TextView>(R.id.profileDepartmentTextView)?.text = 
                                user.branch ?: "Not Set"
                            view.findViewById<TextView>(R.id.profileEmailTextView)?.text = user.email
                            
                            // Set initial
                            val initial = user.fullName.firstOrNull()?.toString()?.uppercase() ?: "H"
                            view.findViewById<TextView>(R.id.profileInitialLargeTextView)?.text = initial

                            // Fill form fields
                            view.findViewById<TextInputEditText>(R.id.fullNameEditText)?.setText(user.fullName)
                            view.findViewById<TextInputEditText>(R.id.emailEditText)?.setText(user.email)
                            view.findViewById<TextInputEditText>(R.id.phoneEditText)?.setText(user.phone)
                            view.findViewById<TextInputEditText>(R.id.departmentEditText)?.setText(user.branch ?: "")
                        }
                    }
                }
            } catch (e: Exception) {
    ...
    private fun saveProfile(view: View) {
        lifecycleScope.launch {
            try {
                val name = view.findViewById<TextInputEditText>(R.id.fullNameEditText)?.text.toString()
                val phone = view.findViewById<TextInputEditText>(R.id.phoneEditText)?.text.toString()
