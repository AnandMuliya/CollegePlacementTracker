        ...
        lifecycleScope.launch {
            try {
                ...
                database.userDao().getAllUsers().observe(this@TPOReportsAnalysisActivity) { users ->
                    ...
                    findViewById<TextView>(R.id.totalStudentsTextView).text = totalStudents.toString()
                    ...
                    lifecycleScope.launch {
                        ...
                        val selectedApplications = applications.filter { it.status == ApplicationStatus.SELECTED }
                        val placedStudents = selectedApplications.map { it.studentId }.distinct().size
                        ...
                        runOnUiThread {
                            ...
                            findViewById<TextView>(R.id.placedStudentsTextView).text = placedStudents.toString()
                            findViewById<TextView>(R.id.totalCompaniesTextView).text = companies.size.toString()
                            findViewById<TextView>(R.id.placementRateTextView).text = "$placementRate%"
                            ...
                            findViewById<TextView>(R.id.totalOffersTextView).text = selectedCount.toString()

                            // Update application status
                            findViewById<TextView>(R.id.pendingCountTextView).text = pendingCount.toString()
                            findViewById<TextView>(R.id.selectedCountTextView).text = selectedCount.toString()
                            findViewById<TextView>(R.id.rejectedCountTextView).text = rejectedCount.toString()
                ...
                e.printStackTrace()
                Toast.makeText(this@TPOReportsAnalysisActivity, "Error loading statistics", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun generateOverallReport() {
        lifecycleScope.launch {
            try {
                ...
                runOnUiThread {
                    if (filePath != null) {
                        Toast.makeText(this@TPOReportsAnalysisActivity, 
                            "Overall report generated! 📊", Toast.LENGTH_LONG).show()
                    } else {
                        Toast.makeText(this@TPOReportsAnalysisActivity, 
            ...
            lifecycleScope.launch {
                try {
                    ...
                    runOnUiThread {
                        if (filePath != null) {
                            Toast.makeText(this@TPOReportsAnalysisActivity, 
                                "Student report generated! 👥", Toast.LENGTH_LONG).show()
                        } else {
                            Toast.makeText(this@TPOReportsAnalysisActivity, 
                                "Failed to generate report", Toast.LENGTH_SHORT).show()
                        }
        ...
        lifecycleScope.launch {
            try {
                ...
                runOnUiThread {
                    if (filePath != null) {
                        Toast.makeText(this@TPOReportsAnalysisActivity, 
                            "Application report generated! 📋", Toast.LENGTH_LONG).show()
                    } else {
                        Toast.makeText(this@TPOReportsAnalysisActivity, 
                            "Failed to generate report", Toast.LENGTH_SHORT).show()
                    }
