...
class TPOInterviewManagementActivity : AppCompatActivity() {
    ...
    private fun loadInterviews() {
        lifecycleScope.launch {
            try {
                ...
                userDao.getAllUsers().observe(this@TPOInterviewManagementActivity) { users ->
                    ...
                    allInterviews = interviews.map { interview ->
                        InterviewAdapter.InterviewWithDetails(
                            interview = interview,
                            companyName = companyMap[interview.companyId]?.companyName ?: "Unknown Company",
                            studentName = userMap[interview.studentId]?.fullName ?: "Unknown Student"
                        )
                    }
                ...
                e.printStackTrace()
                Toast.makeText(this@TPOInterviewManagementActivity, "Error loading interviews", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun loadStatistics() {
        lifecycleScope.launch {
            try {
                ...
                val scheduled = interviewDao.getInterviewCountByStatusSync(InterviewStatus.SCHEDULED)
                val completed = interviewDao.getInterviewCountByStatusSync(InterviewStatus.COMPLETED)

                runOnUiThread {
                    findViewById<TextView>(R.id.totalInterviewsTextView).text = total.toString()
                    findViewById<TextView>(R.id.scheduledInterviewsTextView).text = scheduled.toString()
                    findViewById<TextView>(R.id.completedInterviewsTextView).text = completed.toString()
                }
            } catch (e: Exception) {
                e.printStackTrace()
            }
    ...
    private fun showAddInterviewDialog() {
        ...
        val roundNumberEditText = dialogView.findViewById<TextInputEditText>(R.id.roundNumberEditText)
        ...
        lifecycleScope.launch {
            ...
            companySpinner.adapter = ArrayAdapter(this@TPOInterviewManagementActivity, 
                android.R.layout.simple_spinner_dropdown_item, companyNames)

            userDao.getUsersByRole(UserRole.STUDENT).observe(this@TPOInterviewManagementActivity) { students ->
                val studentNames = students.map { it.fullName }
                studentSpinner.adapter = ArrayAdapter(this@TPOInterviewManagementActivity,
                    android.R.layout.simple_spinner_dropdown_item, studentNames)
            }

            // Setup mode spinner
            val modes = listOf("Online", "Offline")
            modeSpinner.adapter = ArrayAdapter(this@TPOInterviewManagementActivity,
            ...
            roundTypeSpinner.adapter = ArrayAdapter(this@TPOInterviewManagementActivity,
            ...
            .setPositiveButton("Schedule") { _, _ ->
                lifecycleScope.launch {
                    try {
                        ...
                        userDao.getUsersByRole(UserRole.STUDENT).observe(this@TPOInterviewManagementActivity) { students ->
                            if (companySpinner.selectedItemPosition >= 0 && studentSpinner.selectedItemPosition >= 0) {
                                ...
                                val interview = Interview(
                                    ...
                                    interviewRound = roundNumberEditText.text.toString().toIntOrNull() ?: 1,
                                ...
                                lifecycleScope.launch {
                                    ...
                                    loadInterviews()
                                    loadStatistics()
                                    Toast.makeText(this@TPOInterviewManagementActivity, 
                                        "Interview scheduled!", Toast.LENGTH_SHORT).show()
                                }
                            }
                        }
                    } catch (e: Exception) {
                        e.printStackTrace()
                        Toast.makeText(this@TPOInterviewManagementActivity, 
                            "Error scheduling interview", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Yes") { _, _ ->
                lifecycleScope.launch {
                    ...
                    loadStatistics()
                    Toast.makeText(this@TPOInterviewManagementActivity, 
                        "Interview cancelled", Toast.LENGTH_SHORT).show()
                }
            }
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Yes") { _, _ ->
                lifecycleScope.launch {
                    ...
                    loadStatistics()
                    Toast.makeText(this@TPOInterviewManagementActivity, 
                        "Interview marked as completed", Toast.LENGTH_SHORT).show()
                }
            }
            .setNegativeButton("No", null)
            .show()
